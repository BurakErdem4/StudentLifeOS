...
const StudentUI = ({
    ...
    isMentorMode, globalTags, showToast
}) => {
    ...
    const [selectedRadarItems, setSelectedRadarItems] = React.useState([]);
    ...
    // Sync categoryOrder from profile when it changes (persisted across sessions)
    React.useEffect(() => {
        if (profile?.settings?.categoryOrder) {
            setCategoryOrder(profile.settings.categoryOrder);
        }
    }, [profile?.settings?.categoryOrder]);

    React.useEffect(() => {
        if (!projects || projects.length === 0 || selectedRadarItems.length > 0) return;
        ...
        projects.forEach(p => {
            ...
            if (!categories[cat]) categories[cat] = { id: `c_${cat}`, title: cat, type: 'category', totalEstTime: 0, completedEstTime: 0 };
        ...
        Object.values(categories).forEach(c => {
            if (c.totalEstTime > 0) items.push(c);
        });
        ...
        projects.forEach(p => {
            ...
            if (pTotalEstTime > 0) {
                items.push({
                    ...
                    title: p.title,
                    type: 'project',
                    totalEstTime: pTotalEstTime,
                    completedEstTime: pCompletedEstTime
                });
            }
        });
        
        items.sort((a, b) => b.completedEstTime - a.completedEstTime);
        const top5Ids = items.slice(0, 5).map(item => item.id);
        setSelectedRadarItems(top5Ids);
    }, [projects]);
    ...
    const handleImportTemplate = (template) => {
        ...
        showToast && showToast('Görev başarıyla eklendi! 🎯', 'success');
        closeModal();
    };
                        ...
                        <div className="flex items-center gap-4">
                            {t.type === 'project_slice' ? (
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            {!(t.isMentorTask && t.allowDelete === false && !isMentorMode) && (
                                                <button onClick={() => { deleteTask(t.id); showToast && showToast('Görev silindi', 'info'); }} className="text-gray-300 hover:text-red-500 p-1"><Icons.Trash /></button>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <>
                                    ...
                                    {!(t.isMentorTask && t.allowDelete === false && !isMentorMode) && (
                                        <button onClick={() => { deleteTask(t.id); showToast && showToast('Görev silindi', 'info'); }} className="p-2 text-gray-300 hover:text-red-500"><Icons.Trash /></button>
                                    )}
                                </>
                            )}
                        </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar bg-white dark:bg-slate-800 dark:bg-slate-900 transition-colors duration-300">
                ...
                {activeTab === 'projects' && (
                    <div className="p-5 pb-24 space-y-4 animate-fade-in">
                        ...
                        {(() => {
                            // Auto-sort projects: most recently active first (WhatsApp logic)
                            ...
                            const categories = [...allCats].sort((a, b) => {
                                ...
                                if (ai === -1 && bi === -1) return allCats.indexOf(a) - allCats.indexOf(b);
                                if (ai === -1) return 1;
                                if (bi === -1) return -1;
                                return ai - bi;
                            });
                            ...
                            const handleDragEnd = () => {
                                ...
                                if (fromIdx !== -1 && toIdx !== -1) {
                                    ...
                                    // Persist to Firebase under profile/settings/categoryOrder
                                    updateCloud('profile/settings/categoryOrder', newOrder);
                                }
                                dragItem.current = null;
                                dragOverItem.current = null;
                            };
                ...
                {activeTab === 'routines' && (
                    <div className="space-y-6 animate-fade-in pb-24 p-5">
                        ...
                        {/* GRAFİK GEÇİŞ BUTONU VE GÖRÜNÜMLER */}
                        <div className="animate-fade-in">
                            <div className="flex bg-gray-100 dark:bg-slate-800 p-1 rounded-2xl mb-4 max-w-sm mx-auto shadow-inner border border-gray-200 dark:border-slate-700">
                                <button 
                                    onClick={() => setChartType('line')} 
                                    className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs font-bold transition-all ${chartType === 'line' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow shadow-gray-300 dark:shadow-none' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200'}`}
                                >
                                    <span className="text-base">📈</span> Zaman Akışı
                                </button>
                                <button 
                                    onClick={() => setChartType('radar')} 
                                    className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs font-bold transition-all ${chartType === 'radar' ? 'bg-white dark:bg-slate-700 text-purple-600 dark:text-purple-400 shadow shadow-gray-300 dark:shadow-none' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200'}`}
                                >
                                    <span className="text-base">🕸️</span> Yetenek Radarı
                                </button>
                            </div>
                            ...
                            {chartType === 'line' ? (
                                ...
                                <div className="space-y-4">
                                    <StudentRadarChart student={studentData} selectedItems={selectedRadarItems} />
                                    
                                    {/* RADAR Veri Seçici */}
                                    <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700">
                                        <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-3 text-sm flex items-center gap-2">
                                            <span className="text-lg">🎯</span> Radarda Gösterilecekler
                                        </h3>
                                        <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto no-scrollbar">
                                            {(() => {
                                                ...
                                                return allOptions.map(opt => {
                                                    const isSelected = selectedRadarItems.includes(opt.id);
                                                    return (
                                                        <button 
                                                            key={opt.id}
                                                            onClick={() => {
                                                                if (isSelected) {
                                                                    setSelectedRadarItems(prev => prev.filter(id => id !== opt.id));
                                                                } else {
                                                                    setSelectedRadarItems(prev => [...prev, opt.id]);
                                                                }
                                                            }}
                                                            className={`px-3 py-1.5 rounded-xl text-[11px] font-bold transition-all border flex items-center gap-1.5 ${isSelected ? (opt.type === 'category' ? 'bg-purple-100 text-purple-700 border-purple-300 dark:bg-purple-900/40 dark:text-purple-300 dark:border-purple-500/50 shadow-sm' : 'bg-indigo-100 text-indigo-700 border-indigo-300 dark:bg-indigo-900/40 dark:text-indigo-300 dark:border-indigo-500/50 shadow-sm') : 'bg-gray-50 text-gray-400 border-gray-200 hover:bg-gray-100 hover:text-gray-600 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-400 dark:hover:bg-slate-600'}`}
                                                        >
                                                            <span>{opt.type === 'category' ? '📂' : '📌'}</span>
                                                            {opt.title}
                                                        </button>
                                                    );
                                                });
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-sm p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto no-scrollbar">
                        ...
                        {modal.type === 'journal' && (
                            <div className="space-y-4">
                                ...
                                <button onClick={() => { updateCloud(`history/${dateKey}/journal`, { mood: form.mood, note: form.note }); localStorage.setItem('pulseShown_' + todayKey, 'true'); showToast && showToast('Günlük kaydedildi 📓', 'success'); closeModal(); }} className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold">Kaydet</button>
                            </div>
                        )}
                    </div>
                </div>
